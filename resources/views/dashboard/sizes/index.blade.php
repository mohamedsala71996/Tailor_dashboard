            ...
            <div class="card">
                <div class="card-header">
                    @if (auth('user')->user()->has_permission('create-sizes'))
                        <a href="{{ route('dashboard.sizes.create') }}" type="button"
                            ...
                            @foreach ($sizes as $size)
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <button type="button"
                                                class="btn btn-success">{{ trans('admin.Actions') }}</button>
                                            ...
                                            <div class="dropdown-menu" role="menu">
                                                @if (auth('user')->user()->has_permission('update-sizes'))
                                                    <a class="dropdown-item"
                                                        href="{{ route('dashboard.sizes.edit', $size->id) }}">{{ trans('admin.Edit') }}</a>
                                                @endif

                                                @if (auth('user')->user()->has_permission('delete-sizes'))
                                                    <a class="dropdown-item" href="#" data-toggle="modal"
                                                        data-target="#modal-default-{{ $size->id }}">{{ trans('admin.Delete') }}</a>
                                                @endif
                                            </div>
                                        </div>
    ...
    <script>
        $.fn.dataTable.ext.search.push(
            function(settings, data, dataIndex) {
                ...
                var role = $('#role').val();
                if (data[3] === role || role == '') {
                    var role_status = true
                } else {
                    var role_status = false
                };
